    ...
    for i in range(len(input_lines)): #line in input_lines:
        ...
        if len(line_tokens) == 3:
            ...
            if replacement != False:
                ...
                ''' replaces tag for every word starting upper case and with at least 3 letters '''
            elif i>0 and not input_lines[i-1].strip().startswith('<s') and line_tokens[0][0].isupper() and len(line_tokens[0])>2:
                print line_tokens[0] + '\t' + line_tokens[0] + '\tkA'
            else:
